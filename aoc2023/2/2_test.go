...
func SumPossibleGameIDs(input string, r, g, b int) int {
	lines := strings.Split(input, "\n")
	sum := 0
	for _, line := range lines {
		sum += PossibleGameID(line, r, g, b)
	}
	return sum
}

func PossibleGameID(line string, r, g, b int) int {
	if line == "" {
		return 0
	}
	m := regexp.MustCompile(`Game (\d+): `).FindStringSubmatch(line)
	if len(m) < 2 || len(m[1]) == 0 {
		panic("no game id: " + line)
	}
	gameID, err := strconv.ParseInt(m[1], 10, 32)
	if err != nil {
	...
	for _, set := range sets {
		var sr, sg, sb int
		for _, colorInfo := range strings.Split(set, ",") {
			r := regexp.MustCompile(`(\d+) (red|green|blue)`).FindStringSubmatch(colorInfo)
			if len(r) < 1 {
				panic("colorInfo: " + colorInfo)
			}
			ns, cs := r[1], r[2]
			n, err := strconv.ParseInt(ns, 10, 32)
			if err != nil {
				panic(colorInfo + ": strconv: " + err.Error())
			}
			switch cs {
			case "red":
				sr += int(n)
...
func TestIntro(t *testing.T) {
	require.Equal(t, 1, SumPossibleGameIDs(`Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green`, 12, 13, 14))
	require.Equal(t, 0, SumPossibleGameIDs(`Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red`, 12, 13, 14))
...
`
	sum := SumPossibleGameIDs(input, 12, 13, 14)
	require.Equal(t, 8, sum)
}

const AOCInput = `Game 1: 4 red, 18 green, 15 blue; 17 green, 18 blue, 9 red; 8 red, 14 green, 6 blue; 14 green, 12 blue, 2 red
...
func TestPart1(t *testing.T) {
	sum := SumPossibleGameIDs(AOCInput, 12, 13, 14)
	println(sum)
}
...
func TestPart2Intro(t *testing.T) {
	require.Equal(t, 48, SumPower(`Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green`))
	require.Equal(t, 12, SumPower(`Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue`))
	require.Equal(t, 1560, SumPower(`Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red`))
	require.Equal(t, 630, SumPower(`Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red`))
	require.Equal(t, 36, SumPower(`Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`))
	input := `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
