...
pub fn encrypt(key: &[u8], plaintext: &[u8]) -> Vec<u8> {
    ...
    let mut ciphertext = crypto::secretbox::seal(
        plaintext,
...
pub fn decrypt(key: &[u8], ciphertext: &[u8]) -> Vec<u8> {
    ...
    let plaintext = crypto::secretbox::open(
        ciphertext,
        &Nonce::from_slice(nonce).expect("failed making libsodium nonce"),
        &Key::from_slice(key).expect("failed making libsodium key"),
    );
...
pub fn derive_key(master_key: &[u8], context: &[u8]) -> Vec<u8> {
    ...
    let salt =
        argon2id13::Salt::from_slice(&keyed_nonce_size(master_key, context, DERIVEKEY_SALT_LEN))
            .unwrap();
    let mut k = [0; secretbox::KEYBYTES];
    let derived =
        argon2id13::derive_key(&mut k, pw, &salt, DERIVEKEY_OPS_LIMIT, DERIVEKEY_MEM_LIMIT);
