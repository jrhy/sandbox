...
enum Command {
    ...
    Backup {
        ...
        #[arg(long = "scope", value_enum, default_value_t = BackupScope::Current)]
        scope: BackupScope,
        /// destination path
        #[arg(short = 'd')]
...
fn run() -> Result<()> {
    ...
    let endpoint = env::var("S3_ENDPOINT")
        .with_context(|| "S3_ENDPOINT must be set")?
        .parse()
        .with_context(|| "S3_ENDPOINT endpoint must be a valid URL")?;
    let path_style = UrlStyle::Path;
    let region = env::var("AWS_REGION").with_context(|| "AWS_REGION must be set")?;
    let bucket = Bucket::new(endpoint, path_style, (&args.bucket).to_owned(), region)
        .with_context(|| "URL has a valid scheme and host")?;

    // setting up the credentials
    let key = env::var("AWS_ACCESS_KEY_ID").with_context(|| "AWS_ACCESS_KEY_ID must be set")?;
    let secret = env::var("AWS_SECRET_ACCESS_KEY").with_context(|| "AWS_ACCESS_KEY_ID must be set")?;
    let credentials = Credentials::new(key, secret);

    let decryption_key: Option<Bytes> = match args.key {
        Some(ref path) => {
            let bytes = Bytes::from(std::fs::read(path).with_context(|| "read_key")?);
            Some(Bytes::from(
                s3db::node_crypt::derive_key(&bytes, &[]).with_context(|| "derive_key")?,
            ))
        }
        None => None,
    };
    ...
    match args.cmd {
        ...
        Command::Backup {
            ...
            std::fs::create_dir_all(&node_dest).with_context(|| format!("create {}", &node_dest))?;
            ...
            foreach_object(&s3, Some(&root_prefix), |c| {
                ...
                let root = s3db::read_root(&bytes).with_context(|| "read_root")?;
                ...
                let mut f = std::fs::File::create(&root_path)
                    .with_context(|| format!("creating local root {}", &root_path))?;
                f.write(&bytes).with_context(|| "writing root")?;
                Ok(())
            })?;
        }
    }
...
fn vacuum(
    ...
    foreach_object(s3, Some(&merged_root_prefix), |c| {
        ...
        let lastmod = chrono::DateTime::parse_from_rfc3339(&c.last_modified)
            .with_context(|| format!("parse timestamp of {}", c.key))?;
        if lastmod >= cutoff {
...
fn add_new_nodes_from_root(
    ...
    let root = s3db::read_root(&root_bytes).with_context(|| "read_root")?;
    root.mast
        .link
        .as_ref()
        .map(|link| {
    ...
    F: FnMut(&rusty_s3::actions::list_objects_v2::ListObjectsContent) -> Result<()>,
{
    ...
    loop {
        ...
        let res = s3
            ...
            .get(url)
            .send()
            .with_context(|| "HTTP GET")?
            .text()
            .with_context(|| "parse S3 list")?;

        let list = rusty_s3::actions::ListObjectsV2::parse_response(&res)
            .with_context(|| format!("parse ListObjectsV2 response {}", res))?;
...
fn get_object(s3: &S3Cli, key: &str) -> Result<Bytes> {
    let url = s3
        ...
        .sign(s3.duration);
    let response = s3.http_client.get(url).send().with_context(|| "GET")?;
    ...
    response.bytes().with_context(|| "to bytes")
}
        ...
        |e| match e {
            ...
            VisitTreeCaching::DoneVisit(_, maybe_cache) => match maybe_cache {
                Some((bytes, mut file)) => {
                    ...
                    file.write(&bytes)
                        .with_context(|| format!("write node bytes for {}", key))?;
                    Ok(true)
                }
                None => Ok(true),
            },
        ...
        match stack.pop() {
            ...
            Some(Cur::Todo { link }) => {
                ...
                let node = s3db::read_node(&bytes, decryption_key)
                    .with_context(|| format!("read_node from {}", &key))?;
...
fn get_node(
    ...
    let len = file
        .metadata()
        .with_context(|| format!("{}: get file size", file_path))?
        .len();
    ...
    //println!("GET {}", path);
    let response = http_client.get(url).send().context("GET")?;
    if !response.status().is_success() {
        bail!(
    ...
    let bytes = response.bytes().with_context(|| "read HTTP response bytes")?;
    let node = s3db::read_node(&bytes, decryption_key)
        .with_context(|| format!("read_node from {}", file_path))?;
...
fn read_node(
    ...
    file.read_to_end(&mut vec)
        .with_context(|| format!("{}: read cached bytes", file_path))?;
