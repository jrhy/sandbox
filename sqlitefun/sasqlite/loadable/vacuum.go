...
func (c *VacuumModule) Connect(conn *sqlite.Conn, args []string,
	...
	err := declare(`create table sasqlite_vacuum(
		vacuum_error,
		table_name HIDDEN,
		before_time HIDDEN
	)`)
...
func (vc *VacuumCursor) Column(context *sqlite.VirtualTableContext, i int) error {
	...
	case 2:
		context.ResultText(vc.beforeTime.Format(sasqlite.SQLiteTimeFormat))
	}
	return nil
}
...
func (vc *VacuumCursor) Filter(_ int, idxStr string, values ...sqlite.Value) error {
	...
	t, err := time.Parse(sasqlite.SQLiteTimeFormat, values[1].Text())
	if err != nil {
		return fmt.Errorf("before_time: %w (must be like %s)", err, sasqlite.SQLiteTimeFormat)
	}
	vc.beforeTime = t

	vc.vacuumErr = sasqlite.Vacuum(vc.tableName, t)
	return nil
}
